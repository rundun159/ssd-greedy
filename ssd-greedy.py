...
class ssd:
    def __init__(self, ssd_type, num_logical_pages, num_blocks, pages_per_block,
        ...
        if self.ssd_type == self.TYPE_LOGGING:
            ...
            self.not_full_page = np.ones(self.num_blocks,dtype=bool)
        # FTL
        self.forward_map = {}
    ...
    def set_not_full_page(self):
        for i in range(self.num_blocks):
            self.not_full_page[i] = (self.live_page_count[i].block_live_pages < self.pages_per_block)
    ...
    def garbage_collect_greedy(self):
        #@@TH : Lowering cost of sorting by using boolean indexing
        bool_list1 = np.asarray(self.block_status == self.BLOCK_STATE_PROGRAMMED)
        self.set_not_full_page()
        victim_list = sorted(self.live_page_count[bool_list1 * self.not_full_page],key=lambda x: x.block_live_pages, reverse=False)
        blocks_cleaned = 0
